...
@app.route("/search")
def search_cafe_in_location():
    location_query = request.args.get("loc")
    location = db.session.query(Cafe).filter_by(location=location_query).first()
    if location:
        return jsonify(cafes=location.to_dict())
    else:
...
def cafe_closed(cafe_id):
    ...
    if api_key == "TopSecretAPIKey":
        close = db.session.query(Cafe).get(cafe_id)
        if close:
